...
export function TableHead({ className, children, ...props }) {
  return (
    <th className={cn('h-11 px-4 text-left align-middle font-medium text-gray-500', className)} {...props}>
      {children}
    </th>
  )
}
...
export function TableCell({ className, children, ...props }) {
  return <td className={cn('px-4 py-3 align-middle text-gray-700', className)} {...props}>{children}</td>
}
